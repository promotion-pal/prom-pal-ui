import { Slot } from "@radix-ui/react-slot";
import { cva, type VariantProps } from "class-variance-authority";
import { Loader2 } from "lucide-react";
import * as React from "react";
import { cn } from "../../function";
import { PromComponentProps } from "../types";

const buttonVariants = cva(
  "inline-flex items-center cursor-pointer justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-all disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4 shrink-0 [&_svg]:shrink-0 outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive",
  {
    variants: {
      variant: {
        default: "bg-primary text-primary-foreground hover:bg-primary/90",
        destructive:
          "bg-destructive text-white hover:bg-destructive/90 focus-visible:ring-destructive/20 dark:focus-visible:ring-destructive/40 dark:bg-destructive/60",
        outline:
          "border bg-background shadow-xs hover:bg-accent hover:text-accent-foreground dark:bg-input/30 dark:border-input dark:hover:bg-input/50",
        secondary:
          "bg-secondary text-secondary-foreground hover:bg-secondary/80",
        ghost:
          "hover:bg-accent hover:text-accent-foreground dark:hover:bg-accent/50",
        link: "text-primary underline-offset-4 hover:underline",
      },
      size: {
        default: "h-9 px-4 py-2 has-[>svg]:px-3",
        sm: "h-8 rounded-md gap-1.5 px-3 has-[>svg]:px-2.5",
        lg: "h-10 rounded-md px-6 has-[>svg]:px-4",
        icon: "size-9",
        "icon-sm": "size-8",
        "icon-lg": "size-10",
      },
    },
    defaultVariants: {
      variant: "default",
      size: "default",
    },
  },
);

const skeletonVariants = cva("inline-flex rounded-md bg-muted animate-pulse", {
  variants: {
    variant: {
      default: "bg-primary/20",
      destructive: "bg-destructive/20",
      outline: "bg-border",
      secondary: "bg-secondary/50",
      ghost: "bg-accent/50",
      link: "bg-primary/20",
    },
    size: {
      default: "h-9 px-4 py-2",
      sm: "h-8 px-3",
      lg: "h-10 px-6",
      icon: "size-9",
      "icon-sm": "size-8",
      "icon-lg": "size-10",
    },
  },
  defaultVariants: {
    variant: "default",
    size: "default",
  },
});

interface PromButtonProps
  extends
    React.ComponentProps<"button">,
    VariantProps<typeof buttonVariants>,
    PromComponentProps {
  asChild?: boolean;
  loadingText?: string;
}

function PromButton({
  size,
  variant,
  children,
  disabled,
  className,
  loadingText,
  isLoad = false,
  asChild = false,
  skeleton = false,
  ...props
}: PromButtonProps) {
  const Comp = asChild ? Slot : "button";

  if (skeleton) {
    return (
      <div
        data-slot="button-skeleton"
        className={cn(skeletonVariants({ variant, size, className }))}
      >
        <span className="opacity-0">{children}</span>
      </div>
    );
  }

  return (
    <Comp
      data-slot="button"
      className={cn(buttonVariants({ variant, size, className }))}
      disabled={disabled || isLoad}
      {...props}
    >
      {isLoad && (
        <Loader2 className="size-4 animate-spin" data-slot="button-loader" />
      )}
      {isLoad && loadingText ? loadingText : children}
    </Comp>
  );
}

export { PromButton, buttonVariants, skeletonVariants };
